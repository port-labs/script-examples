...
cat blueprints.json | jq -c '.blueprints[]' \
  | while read i; do; \
    ...
    | curl -s -X POST "https://api.getport.io/v1/blueprints" -H 'Content-Type: application/json' -H "Authorization: $NEW_TOKEN" --data-binary @- | jq \
  ; done
...
cat blueprints.json | jq -c '.blueprints[]' \
  | while read i; do; \
    ...
    | curl -s -X PUT "https://api.getport.io/v1/blueprints/$ID" -H 'Content-Type: application/json' -H "Authorization: $NEW_TOKEN" --data-binary @- | jq \
  ; done
